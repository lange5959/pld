...
class NotificationItem(QWidget):
    ...
    def __init__(self, title, message, item, ntype=0, callback=None, sec=10,
                 *args, **kwargs):
    ...
    def mousePressEvent(self, event):
        ...
        if w == self.labelClose:  # 点击关闭图标
            ...
            self._timer.stop()
            self.closed.emit(self.item)
        elif w == self.labelMessage and self.callback and callable(
                self.callback):
            # 点击消息内容
            self._timer.stop()
            self.closed.emit(self.item)
...
class NotificationWindow(QListWidget):
    ...
    @classmethod
    def info(cls, title, message, callback=None, sec=10):
        cls._createInstance()
...
if __name__ == '__main__':
    ...
    layout.addWidget(QPushButton(
        'Info', w,
        clicked=lambda: NotificationWindow.info('提示', '这是一条会自动关闭的消息',
                                                callback=callback, sec=100)))
    layout.addWidget(QPushButton(
        'Success', w,
        clicked=lambda: NotificationWindow.success('提示', '这是一条会自动关闭的消息',
                                                   callback=callback)))
    layout.addWidget(QPushButton(
    ...
    layout.addWidget(QPushButton(
        'Error', w, clicked=lambda: NotificationWindow.error(
            '提示',
            '<html><head/><body><p><span style=" font-style:italic; color:teal;">这是提示文案</span></p></body></html>',
            callback=callback)))
    w.show()
