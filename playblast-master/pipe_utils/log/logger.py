...
from pipe_utils.log.utilities import convert_string
from pipe_utils.log.notification_handler import NotificationLogHandler
...
class _UnicodeLogger(logging.Logger):
    def makeRecord(self, name, level, fn, lno, msg, args, exc_info, func=None,
        ...
        return logging.Logger.makeRecord(self, name, level, fn, lno, msg,
                                         args if convArgs is None else tuple(
                                             convArgs), exc_info, func=func,
                                         extra=extra)
    ...
    def initializeLogger(cls, path):
        if cls.lunch_logger is None:
            ...
            cls.streamHandler.setFormatter(
                logging.Formatter("[%(levelname)7s] [%(pathname)s:%(lineno)d] %(message)s"))
    ...
    def setLoggingLevel(cls, name, newLevel):
        if not name:
            ...
            if oldLevel != newLevel:
                from pipe_utils.log import get_notification_center
                get_notification_center().emitLoggingLevelChanged(name,
                                                                  newLevel)
...
def initializeLogger(path=None):
    ...
    MyLogger.initializeLogger(path)


def getCoreLogger():
    return MyLogger.getCoreLogger()


def newLogger(name):
    return MyLogger.newLogger(name)


def removeLogger(name):
    MyLogger.removeLogger(name)


def setGlobalLoggingLevel(newLevel):
    MyLogger.setGlobalLevel(newLevel)


def setLoggingLevel(loggerName, newLevel):
    MyLogger.setLoggingLevel(loggerName, newLevel)


def getLoggingLevel(component):
    return MyLogger.getLoggingLevel(component)


def getSpecificLoggingLevel(component):
    return MyLogger.getSpecificLoggingLevel(component)


def getCachedLogRecords():
    return MyLogger.getCachedRecords()


def setLogCacheSize(size):
    MyLogger.setCacheSize(size)


def getLoggerNames():
    return MyLogger.getLoggerNames()
...
def serializeLoggingLevels():
    return MyLogger.serialize()


def deserializeLoggingLevels(s):
    MyLogger.deserialize(s)
